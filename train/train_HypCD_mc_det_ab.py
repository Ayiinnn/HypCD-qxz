    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            with torch.cuda.amp.autocast(fp16_scaler is not None):
                ...
                avg_probs = (student_out_cls_unsup / 0.1).softmax(dim=1).mean(dim=0)
        ...
        if epoch:
            ...
            if all_acc > best_train_acc_all:
                ...
                # save best (train all-acc) multi-curvature manager.
                torch.save(curvatures.state_dict(), args.model_path[:-3] + f'_curv_best_acc_all.pt')
                args.logger.info("best (train all-acc) curvatures saved to {}.".format(args.model_path[:-3] + f'_curv_best_acc_all.pt'))

# resolve checkpoint paths for model / projector / classifier / curvatures.
def get_hypsimgcd_ckpt_paths(model_path):
    if model_path.endswith('_best_acc_all.pt'):
        base_path = model_path[:-len('_best_acc_all.pt')]
        return {
            'model': model_path,
            'proj': base_path + '_proj_head_best_acc_all.pt',
            'cls': base_path + '_hyp_cls_best_acc_all.pt',
            'curv': base_path + '_curv_best_acc_all.pt',
        }
