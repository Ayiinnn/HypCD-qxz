...
def train(student, train_loader, test_loader, unlabelled_train_loader, args, hyperbolic_projector, hyperbolic_classifier, curvatures):
    params_groups = get_params_groups(student)
    hyper_params = (list(hyperbolic_projector.parameters()) + list(hyperbolic_classifier.parameters()))

    if args.train_c: 
        hyper_params += list(curvatures.parameters())

    optimizer_hyper = radam_.RiemannianAdam(hyper_params, lr=0.01, stabilize=10)
    ...
    for epoch in range(args.epochs):
        ...
        student.train()
        curvatures.train()
        if args.c_tie_mode == "hard":
            curvatures.hard_sync(epoch, args)
        for batch_idx, batch in enumerate(train_loader):
            ...
            with torch.cuda.amp.autocast(fp16_scaler is not None):
                ...
                sup_logits = torch.cat([f[mask_lab] for f in (student_out_cls_sup / 0.1).chunk(2)], dim=0)
                ...
                cluster_loss = cluster_criterion(student_out_cls_unsup, teacher_out, epoch)
                # ===== [NEW: detach-c-from-memax] BEGIN =====
                # When --detach_c_memax is set, recompute the unsup-classification
                # logits with a detached curvature so the me-max gradient updates the
                # backbone / projector / classifier weights but NOT the curvature
                # c_cls_unsup. Note c_cls_unsup enters BOTH the projection and the
                # classifier, so both are recomputed from `student_out` (which stays
                # attached) with the detached curvature. Otherwise fall back to the
                # original shared `student_out_cls_unsup` (no behaviour change).
                if args.detach_c_memax:
                    c_cls_unsup_det = c_dict["c_cls_unsup"].detach()
                    proj_memax = hyperbolic_projector(student_out, c=c_cls_unsup_det)
                    memax_logits = hyperbolic_classifier(proj_memax, c=c_cls_unsup_det)
                else:
                    memax_logits = student_out_cls_unsup
                avg_probs = (memax_logits / 0.1).softmax(dim=1).mean(dim=0)
                # ===== [NEW: detach-c-from-memax] END =====
                ...
                contrastive_logits, contrastive_labels = info_nce_logits(features=student_proj_rep_unsup, hyp_c=c_dict["c_rep_unsup"], normalize=False)
                ...
                contrastive_logits_angle, contrastive_labels_angle = info_nce_logits(features=student_proj_rep_unsup, hyp_c=0, normalize=False, temperature=args.hyper_temp_scale * 1.0)
                contrastive_loss_angle = torch.nn.CrossEntropyLoss()(contrastive_logits_angle, contrastive_labels_angle)

                # representation learning, sup
                # student_proj = torch.cat([f[mask_lab].unsqueeze(1) for f in student_proj.chunk(2)], dim=1)
                student_proj_rep_sup = torch.cat([f[mask_lab].unsqueeze(1) for f in student_proj_rep_sup.chunk(2)], dim=1)
                sup_con_labels = class_labels[mask_lab]
                # sup_con_loss_distance = SupConLoss(hyp_c=hyp_c)(student_proj, labels=sup_con_labels)
                sup_con_loss_distance = SupConLoss(hyp_c=c_dict["c_rep_sup"])(student_proj_rep_sup, labels=sup_con_labels)

                # sup_con_loss_angle = SupConLoss(hyp_c=0, temperature=0.07 * args.hyper_temp_scale)(student_proj, labels=sup_con_labels)
                sup_con_loss_angle = SupConLoss(hyp_c=0, temperature=0.07 * args.hyper_temp_scale)(student_proj_rep_sup, labels=sup_con_labels)
                ...
                # Add soft curvature tying loss. It should be zero in hard/none mode.
                c_tie_loss = curvatures.tie_loss(epoch, args, device=images.device)
                loss += c_tie_loss
                ...
                # Log curvature tying loss and four effective curvatures.
                pstr += f'c_tie_loss: {c_tie_loss.item():.6f} '
                pstr += curvatures.log_string() + ' '
        ...
        if epoch:
            ...
            all_acc, old_acc, new_acc = test(student, unlabelled_train_loader, epoch, 'Train ACC Unlabelled', args, hyperbolic_projector, hyperbolic_classifier, curvatures)
            ...
            all_acc_test, old_acc_test, new_acc_test = test(student, test_loader, epoch, 'Test ACC', args, hyperbolic_projector, hyperbolic_classifier, curvatures)
            ...
            # save multi-curvature manager.
            torch.save(curvatures.state_dict(), args.model_path[:-3] + f'_curv.pt')
            args.logger.info("curvatures saved to {}.".format(args.model_path[:-3] + f'_curv.pt'))
            ...
            if old_acc_test > best_test_acc_lab:
                ...
                # save best multi-curvature manager.
                torch.save(curvatures.state_dict(), args.model_path[:-3] + f'_curv_best.pt')
                args.logger.info("best curvatures saved to {}.".format(args.model_path[:-3] + f'_curv_best.pt'))

# resolve checkpoint paths for model / projector / classifier / curvatures.
def get_hypsimgcd_ckpt_paths(model_path):
    if model_path.endswith('_best.pt'):
        base_path = model_path[:-len('_best.pt')]
        return {
            'model': model_path,
            'proj': base_path + '_proj_head_best.pt',
            'cls': base_path + '_hyp_cls_best.pt',
            'curv': base_path + '_curv_best.pt',
        }
    else:
        base_path = model_path[:-3]
        return {
            'model': model_path,
            'proj': base_path + '_proj_head.pt',
            'cls': base_path + '_hyp_cls.pt',
            'curv': base_path + '_curv.pt',
        }

def test(model, test_loader, epoch, save_name, args, hyperbolic_projector, hyperbolic_classifier, curvatures):
    model.eval()
...
if __name__ == "__main__":
    ...
    # multi-curvature
    parser.add_argument('--c_tie_mode', default='hard', choices=['none', 'hard', 'soft'])
    # bind rep/cls: In hard mode, > 0 means enabled; in soft mode, it represents the initial lambda value.
    parser.add_argument('--c_tie_rep_cls', type=float, default=1.0)
    # bind sup/unsup: In hard mode, > 0 means enabled; in soft mode, it represents the initial lambda value.
    parser.add_argument('--c_tie_sup_unsup', type=float, default=1.0)
    # unbind
    parser.add_argument('--c_unbind_rep_cls_epoch', type=int, default=25)
    parser.add_argument('--c_unbind_sup_unsup_epoch', type=int, default=25)
    # unfreeze
    ...
    # ===== [NEW: detach-c-from-memax] BEGIN =====
    # If set, detach the unsup-classification curvature (c_cls_unsup) from the
    # me-max loss only. me-max will still update the backbone / projector /
    # classifier weights, but its gradient will NOT flow into the curvature.
    # Default False keeps the original behaviour unchanged.
    parser.add_argument('--detach_c_memax', action='store_true', default=False,
                        help='Detach curvature c_cls_unsup from the me-max loss '
                             '(stop me-max gradients from updating the curvature).')
    # ===== [NEW: detach-c-from-memax] END =====
    ...
    # PROJECTION HEAD
    # ----------------------
    curvatures = hypnn.MultiCurvatureManager(
        init_c=args.c,
        train_c=args.train_c,
    ).to(device)
    args.c = args.c
    if args.cr != 0:
        hyperbolic_projector = hypnn.ToPoincare(c=args.c, train_c=False, ball_dim=args.mlp_out_dim, riemannian=args.riemannian, clip_r=args.cr).to(device)
    else:
        hyperbolic_projector = hypnn.ToPoincare(c=args.c, train_c=False, ball_dim=args.mlp_out_dim, riemannian=args.riemannian).to(device)
    ...
    if args.eval_only:
        if args.eval_model_path is not None:
            ...
            # Resolve all checkpoint paths consistently.
            ckpt_paths = get_hypsimgcd_ckpt_paths(args.eval_model_path)
            model.load_state_dict(torch.load(ckpt_paths['model'], map_location='cpu'))
            hyperbolic_projector.load_state_dict(torch.load(ckpt_paths['proj'], map_location='cpu'))
            hyperbolic_classifier.load_state_dict(torch.load(ckpt_paths['cls'], map_location='cpu'))
            # Load multi-curvature manager if available.
            if os.path.exists(ckpt_paths['curv']):
                curvatures.load_state_dict(torch.load(ckpt_paths['curv'], map_location='cpu'))
                print(f"Loaded curvatures from {ckpt_paths['curv']}")
            else:
                print(f"[MC] No curvature checkpoint found at {ckpt_paths['curv']}. Use initialized curvatures.")

            eval_epoch = args.epochs # use a large enough epoch (parameter)
            test(model, test_loader_unlabelled, eval_epoch, 'Train ACC Unlabelled', args, hyperbolic_projector, hyperbolic_classifier, curvatures)
    else:
        train(model, train_loader, test_loader_labelled, test_loader_unlabelled, args, hyperbolic_projector, hyperbolic_classifier, curvatures)