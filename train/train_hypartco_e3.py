...
def extract_patch_and_cls(backbone, images, model_name):
    """[hypartco-r5] One backbone pass over the part view returning both the
    patch tokens (part losses) and the CLS feature (part pseudo-labels), so the
    total backbone cost stays 3B exactly as in r1-r4 (2B A-views + B part view).
    """
    if model_name == 'v2':
        feats = backbone.forward_features(images)
        return feats['x_norm_patchtokens'], feats['x_norm_clstoken']
    elif model_name == 'v1':
        tokens = backbone(images, return_all_patches=True)
...
def train(student, patch_projector, train_loader, test_loader, unlabelled_train_loader, args,
    ...
    part_unsup_angle = HypPatchUnConLoss(temperature=0.07, hyp_c=0,
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            # [hypartco-r5] views = [A_view_1, A_view_2, part_view]; the image
            # branch consumes A's two original views only (A restored exactly),
            # the part view feeds the part branch below.
            images = [im.cuda(non_blocking=True) for im in images]
            part_images = images[2]
            images = torch.cat(images[:2], dim=0)
            ...
            with torch.cuda.amp.autocast(fp16_scaler is not None):
                ...
                # ------------------------------------------------------------------ #
                # [hypartco-r5] patch tokens and part-view CLS from ONE pass over
                # the aligned part view (previously: re-forward of A's view 0).
                patch_features, part_cls = extract_patch_and_cls(student, part_images, args.model_name)  # [B, N, 768], [B, 768]
                patch_dropout_rng = torch.cuda.get_rng_state()  # [ABLATION E3]
                ...
                if epoch < args.warmup_teacher_temp_epochs:
                    ...
                    patch_sup_con_loss_angle = part_sup_angle(patch_out[mask_lab], patch_label_grid[mask_lab], sup_con_labels)
                    patch_sup_con_loss_dist = part_sup_dist(patch_out[mask_lab], patch_label_grid[mask_lab], sup_con_labels)
                    # [hypartco-r5] pseudo-labels/gate come from the SAME aligned
                    # view as the patch tokens (PartCo's convention: its view 0
                    # served both roles).  no_grad: this path only makes labels.
                    with torch.no_grad():
                        class_logits = hyperbolic_classifier(hyperbolic_projector(part_cls.detach())) / 0.1
                    ...
                    part_unsup_age = epoch  # [ABLATION E3] original global-epoch gate
                    torch.cuda.set_rng_state(patch_dropout_rng)
                    patch_out_unsup = torch.nn.functional.normalize(
                        patch_projector(patch_features.detach()), dim=-1)
                    patch_unsup_con_loss_angle = part_unsup_angle(patch_out_unsup, patch_label_grid, class_logits, mask_lab, epoch=part_unsup_age)
...
if __name__ == "__main__":
    ...
    # [hypartco-r5] three-view A+B contract: the 'contrastive_transform' here is
    # byte-identical to HypCD's original train transform (Resize(size/crop_pct)
    # -> RandomCrop -> RandomHorizontalFlip -> ColorJitter), so A's two global
    # views are restored exactly; PartCo's aligned transform becomes a third,
    # dedicated part view (sync flip handled inside the datasets).
    train_transform = PartCoThreeView(a_transform=contrastive_transform,
                                      part_transform=train_transform)
    # --------------------
    # DATASETS
    # --------------------
