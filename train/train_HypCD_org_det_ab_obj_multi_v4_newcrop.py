...
from models.foreground_v2_attention import DINOv2ForegroundCropper
...
def train(student, train_loader, test_loader, unlabelled_train_loader, args, hyperbolic_projector, hyperbolic_classifier):
    ...
    if args.use_object_branch:
        if args.use_gtbbox:
            ...
            cropper_cls = (DINOv2ForegroundCropper
                           if args.obj_crop_mode == 'v2_attention' else ForegroundCropper)
            fg_cropper = cropper_cls(
...
if __name__ == "__main__":
    ...
    parser.add_argument('--obj_crop_mode', type=str, default='online',
                        choices=['online', 'v2_attention', 'gtbbox'],
                        help="object crop source: original online cropper, DINOv2 last-attention "
                             "cropper, or the existing GT-bbox v2 path.")
    ...
    if args.use_gtbbox:                      # backward-compatible alias
        args.obj_crop_mode = 'gtbbox'
    args.use_gtbbox = args.obj_crop_mode == 'gtbbox'
    if args.obj_crop_mode == 'v2_attention' and args.model_name != 'v2':
        parser.error("--obj_crop_mode v2_attention requires --model_name v2")
    print(args)
