...
import hyptorch.nn_hnnpp as hypnn  # HNN++ MOD: was `import hyptorch.nn as hypnn` (brings in PoincareFC; ToPoincare etc. are unchanged copies from nn_org)
...
if __name__ == "__main__":
    ...
    # HNN++ MOD: classification head is now the HNN++ Poincaré FC layer
    # (Shimizu et al. 2021, Eq. 6-7) instead of the HNN Möbius affine layer.
    # Same signature, same I/O contract (output is a point on B^numclasses whose
    # coordinates are used as logits), so nothing else in this script changes.
    # Original line:
    ...
    hyperbolic_classifier = hypnn.PoincareFC(in_features=args.feat_dim, out_features=args.num_labeled_classes + args.num_unlabeled_classes, c=args.c).to(device)
