...
def set_random_seed(seed: int, deterministic: bool = False, strict: bool = False) -> None:
    ...
    # Safer shell-side equivalent: export CUBLAS_WORKSPACE_CONFIG=:4096:8
    ...
    if deterministic or strict:
        # strict=False/warn_only=True is the practical default for SelEx, because
        # SS-KMeans / kmeans_pytorch can hit non-deterministic CUDA reductions.
...
def train(projection_head, model, train_loader, test_loader, unlabelled_train_loader, merge_train_loader, merge_train_loader_test, args):
    ...
    for epoch in range(args.epochs):
        ...
        # [all-best] additionally save by ALL accuracy on unlabelled train split.
        # In SelEx, all_acc comes from the per-epoch SS-KMeans/prototype extraction
        # except at report epochs, where it is refreshed by test_kmeans().
...
if __name__ == "__main__":
    ...
    # determinism: opt-in; default False keeps original behaviour.
    parser.add_argument('--deterministic', action='store_true', default=False,
                        help='Enable deterministic algorithms with warn_only=True and reproducible dataloading.')
    parser.add_argument('--strict_deterministic', action='store_true', default=False,
                        help='Like --deterministic, but raise on the first non-deterministic CUDA op.')
    # ----------------------
    # INIT
    # ----------------------
