...
def train(student, train_loader, test_loader, unlabelled_train_loader, args, hyperbolic_projector, hyperbolic_classifier, patch_projector=None, probe=None, ema_backbone=None, ema_classifier=None):
    ...
    if getattr(args, 'part_unsup_surgery', 'none') != 'none':
        ...
        patch_unsup_criterion = PatchUnConLossSurgery(temperature=args.part_unsup_temp, dynamic_threshold=True,
                                                      drop={'drop_fp': 'fp', 'drop_fn': 'fn', 'fp_neutral': 'fp_neutral'}[args.part_unsup_surgery])
    elif args.part_unsup_neg_agg == 'min':
        patch_unsup_criterion = PatchUnConLossAllMin(temperature=args.part_unsup_temp, dynamic_threshold=True)
        ...
        patch_unsup_criterion = PatchUnConLossMinDenom(temperature=args.part_unsup_temp, dynamic_threshold=True)
    else:
        patch_unsup_criterion = PatchUnConLoss(temperature=args.part_unsup_temp, dynamic_threshold=True)
            ...
            if args.diag_grad and part_unsup_active and (loss_part_unsup_w is not None) \
                ...
                if getattr(args, 'diag_decomp', False):
                    _ch, _st = decompose_unsup_channels(
                        ...
                        epoch=epoch, temperature=args.part_unsup_temp, base_temperature=0.07,  # [TD] trainer constructs the
                    ...
                    if _st['n_groups'] > 0:
                        ...
                        pstr += (f"dcmp[nTP:{_st['n_TP']} nFP:{_st['n_FP']} nFN:{_st['n_FN']}] "
                                 ...
                                 f"fn_mass: {_st['fn_mass']:.3f} fp_mass: {_st['fp_mass']:.3f} "
                                 f"fp_pull_share: {_st['fp_pull_share']:.3f} "
                                 f"auc_kl: {_st['auc_kl']:.3f} auc_cos: {_st['auc_cos']:.3f} "
                                 f"kl_p50[tp/fp]: {_st['tp_kl_p50']:.3f}/{_st['fp_kl_p50']:.3f} ")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--part_unsup_temp', type=float, default=None,
                        help='Temperature used only by the unsupervised part loss. None inherits --part_temp.')
    ...
    parser.add_argument('--part_unsup_surgery', type=str, default='none', choices=['none', 'drop_fp', 'drop_fn', 'fp_neutral'],
    ...
    parser.add_argument('--part_unsup_scale', type=float, default=1.0,
                        help='Multiplier applied only to the unsupervised part loss.')
    ...
    if args.part_unsup_temp is None:
        args.part_unsup_temp = args.part_temp
    print(args)
