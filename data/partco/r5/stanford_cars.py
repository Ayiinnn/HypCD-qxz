...
from .augmentations import PartCoThreeView, three_view_apply
...
class CarsDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # [hypartco-r5] three-view mode: A's two global views take their own
        # internal crop/flip randomness on the raw image (A baseline contract);
        # the dataset-level sync flip applies only to the part view + label grid.
        if isinstance(self.transform, PartCoThreeView):
            image, patch_label = three_view_apply(
                self.transform, image, patch_label, getattr(self, 'random_hflip', True))
            if self.target_transform is not None:
                target = self.target_transform(target)
            return image, target, patch_label, self.uq_idxs[idx]

        # flip img & patch_label
        # [hypartco] random_hflip switch: True (default) = original PartCo behavior.
        # The trainer sets it to False only on the evaluation datasets so that
        # eval stays deterministic (the *_det_ab contract).
